/* ===============================================
|	//MyMenu library for u8g2 with 2+ lines of text
|   	
|   //Usage:
|   
|   #define NUM_ITEMS 7 //total number of items in menu include submenu-headers
|	#define NUM_ROWS 4 //total number of rows in your u8g2, must be greater than or equal to 2
|
...
|	};
|
|	LiquidCrystal u8g2(...); // your u8g2 initialization
|	String cursor = "-"; // the menu cursor, by default is ">"
|	Menu menu(items,NUM_ITEMS,&u8g2,menuCallback,NUM_ROWS,cursor); //notice &u8g2 instead of u8g2, we will work with pointer
...
Menu::Menu(MItm *_items, int _num_items, U8G2_ST7920_128X64_F_SW_SPI* _u8g2, void (*_callback)(int), int _num_rows, String _cursor){
	items = _items;
	num_rows = _num_rows;
    u8g2 = _u8g2;
	fullsize = _num_items;
	...
	for(int i=0; i<fullsize; i++){
		...
		if(citem.sub_id == cur_sub){
			...
			if(begin_draw && line < num_rows){
				...
				if(i == cur_item){
					(*u8g2).print(cursor+citem.name);
				}else{
					(*u8g2).print(String("                    ").substring(1,cursor.length()+1)+citem.name);
				}
				line++;
			}
		}
